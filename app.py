import osfrom pathlib import Pathfrom datetime import datetimefrom zoneinfo import ZoneInfo

import pandas as pdimport plotly.graph_objects as goimport requestsimport streamlit as stfrom dotenv import load_dotenvfrom streamlit_autorefresh import st_autorefresh
...
st.set_page_config(page_title="SG² MATRIX",layout="wide",initial_sidebar_state="collapsed",)
...
ENV_PATH = Path(file).parent / ".env"load_dotenv(dotenv_path=ENV_PATH, override=True)

TRADIER_API_KEY = os.getenv("TRADIER_API_KEY")TRADIER_BASE_URL = os.getenv("TRADIER_BASE_URL", "https://api.tradier.com/v1")
...
def check_password():if "authenticated" not in st.session_state:st.session_state.authenticated = False
...
SYMBOL_ICONS = {"SPY": "🕷️","SPX": "📈","QQQ": "📊",}

FLOW_DOT_THRESHOLDS = {"SPY": 25_000_000,"SPX": 150_000_000,"QQQ": 25_000_000,}

CHAIN_WIDTH_DEFAULTS = {"SPY": 5,"SPX": 25,"QQQ": 5,}

STRIKE_COUNT_DEFAULTS = {"SPY": 5,"QQQ": 5,"SPX": 25,}
...
if "selected_symbol" not in st.session_state:st.session_state.selected_symbol = "SPX"

if st.session_state.selected_symbol not in SYMBOLS:st.session_state.selected_symbol = "SPX"
...
with st.sidebar:st.markdown("## ⚙️ Dashboard Controls")
...
if auto_refresh:st_autorefresh(interval=refresh_interval * 1000,key="flow_refresh_main",)
...
def fmt_money(value):try:value = float(value)except Exception:return "0"
...
def fmt_num(value):try:value = float(value)except Exception:return "0"
...
def fmt_price(value):try:return f"{float(value):,.2f}"except Exception:return "—"

def metric_html(label, value, color_class=""):return f"""<div><div class="metric-label">{label}</div><div class="metric-value {color_class}">{value}</div></div>"""

def color_class(value):try:value = float(value)except Exception:return ""
...
def safe_get(snapshot, key, default=0):if not isinstance(snapshot, dict):return defaultreturn snapshot.get(key, default)
...
def tradier_get(endpoint, params=None):if not TRADIER_API_KEY:raise Exception("Missing TRADIER_API_KEY in .env")
...
def get_price(symbol):data = tradier_get("markets/quotes",params={"symbols": symbol},)
...
def get_expirations(symbol):data = tradier_get("markets/options/expirations",params={"symbol": symbol,"includeAllRoots": "true","strikes": "false",},)
...
def flatten_greeks(df):if "greeks" not in df.columns:return df
...
def get_option_chain(symbol, expiration):data = tradier_get("markets/options/chains",params={"symbol": symbol,"expiration": expiration,"greeks": "true",},)
...
def filter_near_spot(df, spot, strikes_each_side):"""Keep a fixed number of listed strikes above and below spot.SPY/QQQ: 5 each side. SPX: 25 each side."""if df is None or df.empty or "strike" not in df.columns:return pd.DataFrame()
...
def calculate_net_flow(chain_df, spot=None):if chain_df is None or chain_df.empty:return {"call_premium": 0,"put_premium": 0,"net_premium": 0,"call_delta_notional": 0,"put_delta_notional": 0,"net_delta_notional": 0,"rows": 0,}
...
def get_gamma_levels(chain_df, spot):if chain_df is None or chain_df.empty:return {"top_call_gamma": None,"top_put_gamma": None,}
...
def load_expiration_flow(symbol):spot = get_price(symbol)expirations = get_expirations(symbol)
...
SESSION_DIR = Path(file).parent / "expiration_flow_history"SESSION_DIR.mkdir(exist_ok=True)

def session_file(symbol):return SESSION_DIR / f"expiration_flow_{symbol.upper()}.csv"

def reset_symbol_history(symbol):file_path = session_file(symbol)if file_path.exists():file_path.unlink()

def append_exp_snapshot(flow_data):file_path = session_file(flow_data["symbol"])now = datetime.now(CENTRAL_TZ)
...
def normalize_history(df):df = df.copy()df["datetime"] = pd.to_datetime(df.get("datetime", pd.NaT), errors="coerce")df = df.dropna(subset=["datetime"]).sort_values("datetime")return df

def build_chart_df(history_df):if history_df is None or history_df.empty:return pd.DataFrame()
...
def add_right_edge_label(fig, x, y, text, bg, yref="y", xshift=10):fig.add_annotation(x=x,y=y,text=f"<b>{text}</b>",showarrow=False,font=dict(color="black", size=12, family="Arial Black"),bgcolor=bg,bordercolor=bg,borderwidth=1,borderpad=3,xanchor="left",yanchor="middle",xshift=xshift,yref=yref,)

def add_event_trace(fig, events_df, x_col, y_col, name, marker_color, marker_symbol, text_position, marker_size):"""Add a FLOW event trace and keep the legend item visible even with no events."""if events_df is None or events_df.empty:x_values = [None]y_values = [None]text_values = [""]else:x_values = events_df[x_col]y_values = events_df[y_col]text_values = ["FLOW"] * len(events_df)
...
def build_exposure_df(chain_df, spot):"""Build 0DTE dynamic GEX and DEX by option contract, then chart by strike/type."""if chain_df is None or chain_df.empty:return pd.DataFrame()
...
def plot_exposure_bars(df, spot, symbol, value_col, chart_title, y_title, call_label, put_label):fig = go.Figure()
...
def render_exposure_section(symbol, spot, today_exp, strikes_each_side):try:zero_chain = get_option_chain(symbol, today_exp)zero_chain = filter_near_spot(zero_chain, spot, strikes_each_side)exposure_df = build_exposure_df(zero_chain, spot)except Exception as exposure_error:st.error(f"Could not load GEX / DEX / Vanna charts for {symbol}: {exposure_error}")return
...
def sg2_flow_chart(history_df, symbol, flow_data):df = build_chart_df(history_df)
...
for i, sym in enumerate(SYMBOLS):icon = SYMBOL_ICONS.get(sym, "📊")is_active = sym == st.session_state.selected_symbol
...
symbol = st.session_state.selected_symbolicon = SYMBOL_ICONS.get(symbol, "📊")
...
st.markdown(f"""<div class="active-bar">ACTIVE:   {icon}   {symbol}</div>""",unsafe_allow_html=True,)
...
if reset_exp_history:reset_symbol_history(symbol)st.success(f"{symbol} flow chart history reset.")
...
try:exp_flow_data = load_expiration_flow(symbol)exp_history_df = append_exp_snapshot(exp_flow_data)except Exception as e:st.error(f"Could not load expiration flow chart data for {symbol}: {e}")st.stop()
...
st.plotly_chart(sg2_flow_chart(exp_history_df,symbol,exp_flow_data,),use_container_width=True,config={"displayModeBar": False},)
...
if show_exposure_charts:render_exposure_section(symbol=symbol,spot=exp_flow_data.get("spot", 0),today_exp=exp_flow_data.get("today_exp", ""),strikes_each_side=exposure_strikes_each_side,)
...
try:snapshot = get_flow_snapshot(symbol=symbol,all_exp_count=all_exp_count,chart_bucket=chart_bucket,lookback_hours=lookback_hours,strike_width=chain_width,)except Exception:snapshot = {}

spot = safe_get(snapshot, "spot", exp_flow_data.get("spot", 0))odte_exp = safe_get(snapshot, "odte_exp", exp_flow_data.get("today_exp", ""))

odte_premium_net = exp_flow_data["zero_dte"]["net_premium"]all_exp_premium_net = exp_flow_data["all_exp"]["net_premium"]odte_delta_notional_net = exp_flow_data["zero_dte"].get("net_delta_notional", 0)all_exp_delta_notional_net = exp_flow_data["all_exp"].get("net_delta_notional", 0)

odte_signed_delta = safe_get(snapshot, "odte_signed_delta", 0)odte_delta_bias = safe_get(snapshot, "odte_delta_bias", "NEUTRAL")all_exp_delta_bias = safe_get(snapshot, "all_exp_delta_bias", "NEUTRAL")

gamma_levels = exp_flow_data.get("gamma_levels", {})call_gamma = gamma_levels.get("top_call_gamma") or safe_get(snapshot, "call_gamma", 0)put_gamma = gamma_levels.get("top_put_gamma") or safe_get(snapshot, "put_gamma", 0)gamma_regime = safe_get(snapshot, "gamma_regime", "NEUTRAL")

odte_rows = exp_flow_data["zero_dte"]["rows"]all_exp_rows = exp_flow_data["all_exp"]["rows"]
...
if isinstance(chain_df, pd.DataFrame) and not chain_df.empty and "volume" in chain_df.columns:try:calls = chain_df[chain_df["type"] == "call"]puts = chain_df[chain_df["type"] == "put"]call_volume = calls["volume"].sum()put_volume = puts["volume"].sum()total_volume = call_volume + put_volumepc_ratio = put_volume / call_volume if call_volume > 0 else 0except Exception:call_volume, put_volume, total_volume, pc_ratio = 0, 0, 0, 0else:call_volume, put_volume, total_volume, pc_ratio = 0, 0, 0, 0
...
r1 = st.columns(7)r1[0].markdown(metric_html("Spot", f"{float(spot):.2f}"), unsafe_allow_html=True)r1[1].markdown(metric_html("0DTE Premium Net", fmt_money(odte_premium_net), color_class(odte_premium_net)), unsafe_allow_html=True)r1[2].markdown(metric_html("All Exp Premium Net", fmt_money(all_exp_premium_net), color_class(all_exp_premium_net)), unsafe_allow_html=True)r1[3].markdown(metric_html("0DTE Δ Notional", fmt_money(odte_delta_notional_net), color_class(odte_delta_notional_net)), unsafe_allow_html=True)r1[4].markdown(metric_html("All Exp Δ Notional", fmt_money(all_exp_delta_notional_net), color_class(all_exp_delta_notional_net)), unsafe_allow_html=True)r1[5].markdown(metric_html("Call Gamma", fmt_price(call_gamma), "green-text"), unsafe_allow_html=True)r1[6].markdown(metric_html("Put Gamma", fmt_price(put_gamma), "red-text"), unsafe_allow_html=True)

st.markdown("<hr>", unsafe_allow_html=True)

r2 = st.columns(10)r2[0].markdown(metric_html("0DTE Signed Delta", fmt_money(odte_signed_delta), color_class(odte_signed_delta)), unsafe_allow_html=True)r2[1].markdown(metric_html("0DTE Delta Bias", odte_delta_bias, "red-text" if "BEAR" in str(odte_delta_bias) else "green-text"), unsafe_allow_html=True)r2[2].markdown(metric_html("All Exp Delta Bias", all_exp_delta_bias, "red-text" if "BEAR" in str(all_exp_delta_bias) else "green-text"), unsafe_allow_html=True)r2[3].markdown(metric_html("Gamma Regime", gamma_regime, "green-text" if "ABOVE" in str(gamma_regime) else "red-text"), unsafe_allow_html=True)r2[4].markdown(metric_html("0DTE Rows", odte_rows), unsafe_allow_html=True)r2[5].markdown(metric_html("All Exp Rows", all_exp_rows), unsafe_allow_html=True)r2[6].markdown(metric_html("Call Vol", fmt_num(call_volume), "green-text"), unsafe_allow_html=True)r2[7].markdown(metric_html("Put Vol", fmt_num(put_volume), "red-text"), unsafe_allow_html=True)r2[8].markdown(metric_html("Total Vol", fmt_num(total_volume)), unsafe_allow_html=True)r2[9].markdown(metric_html("P/C Ratio", f"{pc_ratio:.2f}", "yellow-text"), unsafe_allow_html=True)
